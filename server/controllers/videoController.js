...
export const updateVideo = async (req, res, next) => {
  try {
    ...
    if (req.user.id === video.userId) {
      ...
      return next(customError(403, "You can update only your video!"));
    }
  } catch (err) {
    next(err);
  }
...
export const deleteVideo = async (req, res, next) => {
  try {
    ...
    if (req.user.id === video.userId) {
      ...
      return next(customError(403, "You can delete only your video!"));
    }
  } catch (err) {
    next(err);
  }
...
export const userVideos = async (req, res, next) => {
  try {
    ...
    const list = await Promise.all(await Video.find({ userId: Id }));
